...
async def login(page, username: str, password: str):
    ...
    cookie_btn = page.get_by_test_id("login-button")
...
async def gpt_chat_chatgpt_com(page, prompt: str, observe: str, _timeout: int, username: str, password: str):
    # Perform authentication step
    await login(page, username, password)
    
    # Locate message input text area
    input_selector = page.locator("#prompt-textarea").first
    # input_selector = page.get_by_role("textbox", name="Chat with ChatGPT")
    ...
    while attempts < max_attempts:
        ...
        response_container = page.locator('[data-message-author-role="assistant"]').last
...
async def main():
    ...
    async with InvisiblePlaywright() as browser:
        ...
        result = await gpt_chat_chatgpt_com(
            page=page, 
            prompt=prompt,
            observe=observe,
            _timeout=3000,
