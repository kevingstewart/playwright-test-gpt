...
async def gpt_chat_copilot_com(page, prompt: str, observe: str, _timeout: int, username: str, password: str):
    # Perform authentication step
    await login(page, username, password)
    
    # Locate message input text area
    input_selector = page.get_by_role("textbox", name="Message Copilot")
    ...
    while attempts < max_attempts:
        ...
        response_container = page.get_by_test_id("ai-message-body")
...
async def main():
    ...
    async with InvisiblePlaywright() as browser:
        ...
        result = await gpt_chat_copilot_com(
            page=page, 
            prompt=prompt,
            observe=observe,
            _timeout=3000,
