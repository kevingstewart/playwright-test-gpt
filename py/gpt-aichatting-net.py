...
async def gpt_aichatting_net(page, prompt: str, observe: str, _timeout: int):
    # Locate the first textarea matching the specific slot attribute
    input_selector = page.get_by_role('textbox', name='Type a message...')
    ...
    while attempts < max_attempts:
        ...
        response_container = page.locator('[class*="ChatBoxMessage_answerWrapper"]:visible p').last
...
async def main():
    ...
    async with InvisiblePlaywright() as browser:
        ...
        await page.goto("https://aichatting.net/", timeout=60000)

        result = await gpt_aichatting_net(
            page=page, 
            prompt=prompt,
            observe=observe,
            _timeout=3000
